...
We'd love for you to contribute to our source code and to make `twilio-csharp`
...
Help us keep `twilio-csharp` open and inclusive. Please be kind to and considerate
of other developers, as we all have the same goal: make `twilio-csharp` as good as
it can be.
...
* **Major Changes** that you wish to contribute to the project should be
  discussed first with `twilio-csharp` contributors in an issue or pull request so
...
* Run the full `twilio-csharp` test suite (aliased by `make test`), and ensure
  that all tests pass.
...
In GitHub, send a pull request to `twilio-csharp:master`.
