...
namespace Twilio.Tests.Api.V2010.Account {
    ...
    public class ConferenceTest : TwilioTest {
        ...
        public void TestFetchRequest() {
            ...
            try {
                var task = ConferenceResource.Fetch("ACaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "CFaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa").ExecuteAsync(twilioRestClient);
            task.Wait();
        ...
        public void TestReadRequest() {
            ...
            try {
                var task = ConferenceResource.Read("ACaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa").ExecuteAsync(twilioRestClient);
            task.Wait();
                Assert.Fail("Expected TwilioException to be thrown for 500");
            } catch (AggregateException ae) {
                ae.Handle((e) =>
