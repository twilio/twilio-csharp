...
SONAR_SOURCES ?= /d:sonar.sources=src/Twilio/Rest/**/* /d:sonar.tests=test/Twilio.Test/**/*.*
clean:
	dotnet clean
...
cover:
	dotnet sonarscanner begin /k:"$(PROJECT_NAME)" /o:"twilio" /d:sonar.scm.disabled=true /d:sonar.host.url=http://host.docker.internal:9000 /d:sonar.login="${SONAR_TOKEN}"  $(SONAR_SOURCES) /d:sonar.cs.opencover.reportsPaths="test/lcov.net451.opencover.xml"
