    ...
    public class DailyResource : Resource 
    {
        ...
        private static Request BuildReadRequest(ReadDailyOptions options, ITwilioRestClient client)
        {
            return new Request(
                HttpMethod.Get,
                Rest.Domain.Api,
                "/2010-04-01/Accounts/" + (options.AccountSid ?? client.AccountSid) + "/Usage/Records/Daily.json",
                client.Region,
        ...
        public static ResourceSet<DailyResource> Read(ReadDailyOptions options, ITwilioRestClient client = null)
        {
            ...
            var page = Page<DailyResource>.FromJson("usage_records", response.Content);
        ...
        public static async System.Threading.Tasks.Task<ResourceSet<DailyResource>> ReadAsync(ReadDailyOptions options, ITwilioRestClient client = null)
        {
            client = client ?? TwilioClient.GetRestClient();
        ...
        {
            var options = new ReadDailyOptions{AccountSid = accountSid, Category = category, StartDate = startDate, EndDate = endDate, PageSize = pageSize, Limit = limit};
